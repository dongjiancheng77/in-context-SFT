...
def greedy_tsp(embeddings):
    ...
    for x in range(1, n):
        ...
        while True:
            valid = True
            # 检查路径中最后1, 2, 3, 4个点
            for back in range(1, min(1, len(path)+1)):
...
def main(input_file, output_file):
    ...
    if os.path.exists("bert_embeddinggsmbert.npy"):
        text_embedding = np.load("bert_embeddinggsmbert.npy")
        print(1)
    else:
        text_embedding = bert_embedding(instruction_list)
        print(2)
        np.save("bert_embeddinggsmbert.npy",text_embedding)
    # 打印原始顺序的距离
