...
def main(input_file, output_file):
    ...
    instruction_list = [d["instruction"]+d["input"]+d["output"] for d in data]
    print('Processing instructions...')
    if os.path.exists("bert_embeddinggsmio_bert.npy"):
        text_embedding = np.load("bert_embeddinggsmio_bert.npy")
        print(1)
    else:
        text_embedding = bert_embedding(instruction_list)
        print(2)
        np.save("bert_embeddinggsmio_bert.npy",text_embedding)
    # 打印原始顺序的距离
