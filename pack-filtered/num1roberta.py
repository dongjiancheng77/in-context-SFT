...
def bert_embedding(texts, batch=100):
    ...
    tokenizer = RobertaTokenizer.from_pretrained('/home/nfs04/dongjc/roberta-large')
    model = RobertaModel.from_pretrained('/home/nfs04/dongjc/roberta-large').cuda()

    # Encode the texts and prepare for model input
    ...
    while i < len(texts):
        # Process in batches
        outputs = model(input_ids=encoded_texts["input_ids"][i:i+batch],
                        attention_mask=encoded_texts["attention_mask"][i:i+batch])
        last_hids = outputs.last_hidden_state
        cls_hids = last_hids[:, 0, :].squeeze()  # Get the CLS token hidden state
        cls_hid_li.append(cls_hids)
        i += batch
        print(i)
    cls_hids_tensor = torch.concat(cls_hid_li, dim=0)
    return np.array(cls_hids_tensor.cpu())  # Convert to NumPy array for further processing
...
def greedy_tsp(embeddings):
    ...
    for x in range(1, n):
        ...
        max_attempts = 5000  # 设置一个最大尝试次数
        attempts = 0  # 初始化尝试次数计数器

        while True:
            valid = True
            # 检查路径中最后1, 2, 3, 4个点
            for back in range(1, min(1, len(path))):
...
def main(input_file, output_file):
    ...
    if os.path.exists("bert_embeddinggsmreberta.npy"):
        text_embedding = np.load("bert_embeddinggsmreberta.npy")
        print(1)
    else:
        text_embedding = bert_embedding(instruction_list)
        print(2)
        np.save("bert_embeddinggsmreberta.npy",text_embedding)
    # 打印原始顺序的距离
